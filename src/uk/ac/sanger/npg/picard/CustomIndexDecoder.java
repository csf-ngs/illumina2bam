 ...
 * 
 * @author gq1@sanger.ac.uk, ido.tamir@imp.ac.at
 * 
 */

public class CustomIndexDecoder extends PicardCommandLine {
    
    private final Log log = Log.getInstance(CustomIndexDecoder.class);
    
    private final String programName = "CustomIndexDecoder";
    ...
    @Option(doc="split on barcode in 2nd read", optional=true)
    public boolean SPLIT_ON_SECOND = false;
    ...
    protected int doWork() {
        ...
        while(inIterator.hasNext()){
            ...
            if(isPaired){
                ...
                if(SPLIT_ON_SECOND){
                   barcodeRead = pairedRecord.getReadString().substring(0,barcodeLength);
                }
         ...
         //       }                
            ...
            record.setAttribute(BARCODE_TAG_NAME, barcode);
            
            if(match.matched){
              record.setReadString(record.getReadString().substring(barcodeLength));
              record.setBaseQualityString(record.getBaseQualityString().substring(barcodeLength));
            }
            ...
            if (isPaired) {
                ...
                String cutPair = "";
                if(match.matched){
                   cutPair = pairedRecord.getReadString().substring(0,barcodeLength);
                   pairedRecord.setReadString(pairedRecord.getReadString().substring(barcodeLength));
                   pairedRecord.setBaseQualityString(pairedRecord.getBaseQualityString().substring(barcodeLength));
                }
                pairedRecord.setAttribute("RC", cutPair);                
            }
    ...
    public static void main(final String[] argv) {
        System.exit(new CustomIndexDecoder().instanceMain(argv));
    }
